...
function buildBudgetProposal(userId, message) {
  const amount = parseAmount(message);
  if (amount <= 0) return null;

  const category = findCategory(message);
  const isCategoryBudget = /\b(for|on)\b/i.test(message) && category !== 'Other';
...
export function buildAssistantActionProposal(userId, message, history = []) {
  ...
  if (isBudgetRequest(cleanMessage)) return buildBudgetProposal(userId, cleanMessage);
